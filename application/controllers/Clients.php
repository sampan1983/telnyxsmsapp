...
//			$this->db->truncate('tapp_sent_msg_log');
	...
	$last_name = trim($allDataInSheet[$i]["B"]);
	$name2=str_replace("(", "", $last_name);
	...
	$candidate_location = trim($allDataInSheet[$i]["E"]);
	$candidate_location = preg_replace('/[^a-zA-Z0-9_ -]/s',' ',$candidate_location);



	$job_title = trim($allDataInSheet[$i]["F"]);
