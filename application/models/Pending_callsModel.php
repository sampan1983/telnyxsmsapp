...
use Telnyx\CallControlApplication;
	...
	{		
		...
		{ 
...

			{
				...
				$url = base_url().'voice_upload/'.$get_sid[$i]['voice_file'];

				$ans_url=base_url().'test2.php?audio='.$get_sid[$i]['voice_file'];

       



$to_plus='+'.str_replace('+','',$to);

$from_plus='+'.str_replace('+','',$from);
