...
$check_twilio_number = "select * from tapp_twilio_account ";
 ...
 $counter = 0;
	 ...
	 $fname = $clients_name[$i]['first_name'];
	 $lname = $clients_name[$i]['lastname'];
	 $phone = $clients_name[$i]['sender'];
	 $job_title = $clients_name[$i]['job_title'];



$message1=str_replace('{{firstname}}',$fname,$message);
$message2=str_replace('{{lastname}}',$lname,$message1);
$message3=str_replace('{{phone}}',$phone,$message2);
$message2=str_replace('{{jobtitle}}',$job_title,$message3);
	 ...
	 if ($check_tapp_sent_msg_result->num_rows()<1) {
		...
		if($sending_type=='scheduled'){
			$insert_msg_log = "INSERT INTO tapp_sent_msg(service_type, sms_number, message, twilio_num, images, bulk_name,date_time,scheduled_time,user_id,sender_type,message_profile_id) VALUES ('twilio','$nmbr','$message2','$twilio_number','$img','$name',now(),'$scheduled_time','".$_SESSION['id']."','$type','$message_profile_id')";
		}
		else{
...
else
{
 	...
 	for ($i=0; $i < $count ; $i++)
 	{
   	$sql = "select * from tapp_tbl_clients where user_id = '".$_SESSION['id']."' and sender='".$clients_name[$i]."'";
	 $result = $this->db->query($sql);
	 $row = $result->num_rows();
	...
	{
 	 $name_data = $result->result_array();
// 	 $fname = $name_data[0]['first_name'];
// 	 $lname = $name_data[0]['last_name'];
...
// $message2=str_replace('{{LastName}}',$lname,$message1);
	 ...
	 $fname = $name_data[0]['first_name'];
	 $lname = $name_data[0]['lastname'];
	 $phone = $name_data[0]['sender'];
	 $job_title = $name_data[0]['job_title'];
...
$message2=str_replace('{{jobtitle}}',$job_title,$message3);
	 ...
	 // if ($check_tapp_sent_msg_result->num_rows()<1) {
		 ...
		 // echo $sending_type;
		if($sending_type=='scheduled'){
			$insert_msg_log = "INSERT INTO tapp_sent_msg(service_type, sms_number, message, twilio_num, images, bulk_name,date_time,scheduled_time,user_id,sender_type,message_profile_id) VALUES ('twilio','$clients_name[$i]','$message2','$twilio_number','$img','$name',now(),'$scheduled_time','".$_SESSION['id']."','$type','$message_profile_id')";
		}
		else{
...
// $fname  = $tapp_groups_data[$i]['first_name'];
// $lname  = $tapp_groups_data[$i]['last_name'];
...
// $message2=str_replace('{{LastName}}',$lname,$message1);
	 ...
	 $fname = $tapp_groups_data[$i]['first_name'];
	 $lname = $tapp_groups_data[$i]['last_name'];
	 $phone = $tapp_groups_data[$i]['number'];
	 $job_title = $tapp_groups_data[$i]['job_title'];
...
$message2=str_replace('{{jobtitle}}',$job_title,$message3);
		 ...
		 if($sending_type=='scheduled'){
			$insert_msg_log = "INSERT INTO tapp_sent_msg(service_type,sms_number,message,twilio_num,images,bulk_name,date_time,scheduled_time,user_id,sender_type,message_profile_id) VALUES ('twilio','$number','$message2','$twilio_number','$img','$group_name',now(),'$scheduled_time','".$_SESSION['id']."','$type','$message_profile_id')";
		}
		else{
			$insert_msg_log = "INSERT INTO tapp_sent_msg(service_type,sms_number,message,twilio_num,images,bulk_name,date_time,scheduled_time,user_id,sender_type,message_profile_id) VALUES ('twilio','$number','$message2','$twilio_number','$img','$group_name',now(),'$scheduled_time','".$_SESSION['id']."','$type','$message_profile_id')";
...
for($i=1;$i<=$arrayCount;$i++){
 ...
 $fname = trim($allDataInSheet[$i]["B"]);
  ...
  $jobtitle = trim($allDataInSheet[$i]["D"]);
...
$message1=str_replace('{{firstname}}',$fname,$message);
$message2=str_replace('{{lastname}}',$lname,$message1);
$message3=str_replace('{{phone}}',$sms_number,$message2);
$message2=str_replace('{{jobtitle}}',$jobtitle,$message3);

	try
	{
		if($sending_type=='scheduled'){
			$insert_msg_log = "INSERT INTO tapp_sent_msg(service_type,sms_number,message,twilio_num,images,bulk_name,date_time,scheduled_time,user_id) VALUES ('twilio','$sms_number','$message2','$twilio_number','$img','file',now(),'$scheduled_time','".$_SESSION['id']."')";
		}
		else{
